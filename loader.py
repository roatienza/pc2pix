...
def read_view(f, idx):
    ...
    for line in lines:
        line = line.strip()
        angle = [float(x) for x in line.split(" ")]
        angles.append(angle)

    fh.close()
    # angles = np.array(angles)[:, 0:2] / 180.0
    # azimuth (index 0) must be normed by 180 while elevation (index 1) by 40
    angles = np.array(angles)[:, 0:2] / [180.0, 40.0]
    return angles[idx]

def read_view_angle(f, idx, elev=True):
    fh = open(f, "r")
    lines = fh.readlines()
    ...
    fh.close()
    if elev:
        angles = np.array(angles)[:, 1:2] / 40.0
    else:
        angles = np.array(angles)[:, 0:1] / 180.0
