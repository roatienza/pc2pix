...
def ssim(img1, img2, max_val=255, filter_size=11,
  ...
  max1 = np.amax(sigma11)
  max2 = np.amax(sigma22)
  sigma11 = np.clip(sigma11, 0, max1)
  sigma22 = np.clip(sigma22, 0, max2)
  #sigma1 = np.sqrt(np.abs(sigma11))
  #sigma2 = np.sqrt(np.abs(sigma22))
  sigma1 = np.sqrt(sigma11)
  ...
  # contrast
  nc = 2.0 * sigma1 * sigma2 + c2
  contrast = nc / v2
  # cs = np.mean(v1 / v2)

  # luminance
  luminance = ((((2.0 * mu12 + c1)) / ((mu11 + mu22 + c1))))

  structure = v1 / nc
  sim = np.mean(luminance * contrast * structure)
